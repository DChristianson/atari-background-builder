...
const modes = {
  ...
  scrollAsmPFMirroredx2: {
    ...
    MULTICOLOR: true,
    DOWNLOADS: {
      "asm": { file: "r9PF.asm", action: (file) => { download(file, codeASMpfAsymRepeatedScrollable(2)); } },
      "macro": { file: "macro.h", action: downloadMacro },
      "vcs": { file: "vcs.h", action: downloadVCS }
    },
    NAME: 'ASM Scrollable Playfield (2 scanline)',
    DESCRIPTION: 'Scrollable Asymmetrical Playfield (Repeated) kernel, per scanline color change.',
    OVERLAYS: [
      () => { drawPFOverlay() },
      () => { drawHOverlay(16) },
      () => { drawTestSprite() },
